...
const templates = {
  ...
  envelope: pug.compileFile('./lib/templates/envelope.pug'),
};
...
class Region {
  ...
  get envelope() {
    const result = [];
    const events = this.session.doc(this.element)
                        .find('> Envelope AutomationList[automation-id="envelope"] events')
                        .text();

    if (events.length) {
      const tokens = events.trim().split(/\s+/);
  ...
  set envelope(envelope) {
    let data = "";
    envelope.forEach((event) => {
      let [when, value] = event;
      if (typeof when == "string")
        when = this.session.tcToSample(when);

      data += `${when} ${value}\n`;
    });

    const fragment = cheerio.load(templates.envelope({ doc: this.session }), { xmlMode: true })('Envelope');
    fragment.find('events').text(data);
    this.session.doc(this.element).find('> Envelope').replaceWith(fragment);
  }
}
...
//    this.session.doc(element).find('Region').each((idx,elem) => this.regions.set(elem.attribs['id'], elem));
  ...
  region(id) {
    const region = this.session.doc(this.element).find(`Region[id="${id}"]`).get(0);
    if (!region)
      return undefined;

    return new Region(this.session, region);
  }
  ...
  makeRegion(name, start, length, position, sources) {
    ...
    return new Region(this.session, region.get(0));
  }
}
...
class ArdourXML {
  ...
  newStereoRoute(routeName) {
    return this._newRoute(routeName, templates.stereoRoute);
  }

  newMonoRoute(routeName) {
    return this._newRoute(routeName, templates.monoRoute);
  }

  newRoute(routeName, channels) {
    switch(channels) {
      case 1: return this.newMonoRoute(routeName);
      case 2: return this.newStereoRoute(routeName);
      default: throw new Error(`Does not support route with ${channels} channels`);
    }
  }
